        ...
        gmm_common_state_e state)
{
    ...
    case AMF_EVENT_5GMM_MESSAGE:
        ...
        case OGS_NAS_5GS_IDENTITY_RESPONSE:
            ...
            nas_message->gmm_cause = gmm_handle_identity_response(amf_ue, &nas_message->gmm.identity_response);

            if (gmm_cause != OGS_5GMM_CAUSE_REQUEST_ACCEPTED) {
                ogs_error("gmm_handle_identity_response() failed [%d]", gmm_cause);
                r = nas_5gs_send_service_reject(ran_ue, amf_ue, gmm_cause);
                ogs_expect(r == OGS_OK);
                ogs_assert(r != OGS_ERROR);
                OGS_FSM_TRAN(s, gmm_state_exception);
            ...
            if (!AMF_UE_HAVE_SUCI(amf_ue)) {
                ogs_info("Identity response : AMF UE don't have SUCI");
                r = nas_5gs_send_service_reject(ran_ue, amf_ue, gmm_cause);
                ogs_expect(r == OGS_OK);
                ogs_assert(r != OGS_ERROR);
                OGS_FSM_TRAN(s, gmm_state_exception);
